...
{
  my $logger = Log::Dis::Patchy::Ouli->new_tester({
    log_pid => 1,
    ident   => 't/basic.t',
  });

  isa_ok($logger, 'Log::Dis::Patchy::Ouli');
...
{
  my $logger = Log::Dis::Patchy::Ouli->new({
    ident   => 'foo',
    to_self => 1,
    log_pid => 0,
  });
...
{
  my $logger = eval { Log::Dis::Patchy::Ouli->new; };
  # like($@, qr/no ident specified/, "can't make a logger without ident");
  # Moo's required attr error message is different
  like($@, qr/Missing required arguments: ident at.*/,
       "can't make a logger without ident");
}
...
{
  my $logger = Log::Dis::Patchy::Ouli->new_tester({ debug => 1 });
